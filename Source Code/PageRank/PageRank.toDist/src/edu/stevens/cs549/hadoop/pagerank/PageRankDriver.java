...
public class PageRankDriver {
	...
	//	public static final String CACHE_DIR = "<LOCAL>";
	//	public static final String CACHE_DIR = "/cache"; //local cache
	
	public static final String CACHE_DIR = "s3://cs-549-assignment5/name"; // for EMR
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		System.out.println(" setMapper Excecution Completed");
		System.out.println(DiffMap1.class);
		
		job.setReducerClass(DiffRed1.class);
		System.out.println("setReducer Execution Completed");
		System.out.println(DiffRed1.class);
		
		job.setMapOutputKeyClass(Text.class); // Sets output classes for Mapper and Reducers for Job 1
		System.out.println("setMapOutputKey Completed");
		job.setMapOutputValueClass(Text.class);
		System.out.println("setMapOutputValue Completed");
		job.setOutputKeyClass(Text.class);
	...
	static void finish(String ip, String op, int reducers) throws Exception {
		finishJoin(ip, op, reducers);
		
		// Exits once job finishes.
		deleteDirectory(ip); // Deletes the input directory
	}
	
	static void finishJoin(String input, String output, int reducers)
			throws Exception {
		String temp_Join_Dir = "tempJoin";
		System.out.println("Finish Job Started");
		Job j = Job.getInstance(); // Creates a new Job
		j.setJarByClass(PageRankDriver.class); // Sets the Driver class
		j.setNumReduceTasks(reducers); // Sets the number of reducers

		FileInputFormat.addInputPath(j, new Path(input)); // Adds input and output paths
		//Input
		FileInputFormat.addInputPath(j, new Path(CACHE_DIR)); 
		FileOutputFormat.setOutputPath(j, new Path(temp_Join_Dir));
		// Sets Mapper and Reducer Classes
		j.setMapperClass(FindJoinMapper.class); 
		j.setReducerClass(FindJoinReducer.class);
		// Sets Mapper and Reducer output types
		...
		if(j.waitForCompletion(true)) {
			finishCombine(temp_Join_Dir, output, reducers);
		}
	}
	...
	public static void composite(String input, String output, String interim1,
		 ...
		 * TODO 
		 */
		System.out.println("Abhilash Ugaonkar (10415787)");
		...
		while (difference >= MAX_DIFFERENCE) // continues operation till difference is of specified value
		{
			if (i % 2 == 0) {
	...
	static void summarizeResult(String path) throws Exception {
		...
		Map<Long, String> nodeNames = new HashMap<Long, String>();
		...
		if (fs.exists(finpath)) {
				...
				if (file.getPath().getName().startsWith("part-r-00")) {
					...
					while (i <= 10 && diffcontent != null) // Adds only 10 values max. per file
					{
						...
						if (diffcontent != null) // Adds new value as long as value exists
						{
							...
							String[] section = parts[0].split("\\+");
							long node = Long.parseLong(section[0]);
							String nodeName = "0";
							if(section.length > 1) {
								nodeName = section[1];
							}
			...
			try {
				 ...
				 */
				for (int i = 0; i < nodes.length && i < MAX_OUTPUT; i++) {
					
					String out = nodeNames.get(nodes[i]) + "\t" + ranks[i] + "\n";
					/*
	...
	static void deleteDirectory(String path) throws Exception {
		...
		System.out.println("Path to be deleted: ");
